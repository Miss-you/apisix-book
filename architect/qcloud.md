...
考虑完路由配置的一致性，那就需要关心配置中心的高可用问题，由于 Apache APISIX 当前的配置中心使用的是 etcd，也就意味着 etcd 的集群化部署以及 APISIX 转发节点以及 Admin API 如何连接 etcd 集群。网上有很多文章介绍 etcd 集群如何部署以及一些运维技巧，我们在这里就不作过多赘述；重点讲一下 APISIX 等节点连接 etcd 集群的方案对比。

连接 etcd 集群有两种做法，一是通过 APISIX 配置，配置 etcd 集群，另一个则是 APISIX 节点上同时部署一个 etcd proxy 组件，在etcd 代理的模式下，etcd 作为反向代理，将 APISIX 请求转发到 etcd 集群中，etcd 代理不参与 etcd 集群的一致性复制。这两种方案各有优劣，比如方案一简洁，方案二则让 etcd 集群对于 APISIX 是透明的，只需要关注从 etcd 代理拉去相关路由规则即可；另一方面，由于 Openresty 基础库不支持客户端发起 https 双向认证，最近也有用户咨询 APISIX 如何支持 etcd 双向认证，采用方案二则可以通过Etcd proxy 完成与 Etcd集群 的双向认证，一举多得，所以这里的最佳实践是第二种方法，APISIX 转发节点和 Admin API 节点通过本地 etcd proxy 连接 etcd 集群。

安全性是很多人都会忽视的点，但边缘网关往往会存储很多秘钥信息以及证书信息，如果不注意，一旦被黑客入侵，则会给公司造成巨大损失，追悔莫及。生产环境要做好安全防护，就像人类需要打疫苗一样，防患于未然。控制面的安全问题往往是 Admin API 会在没有防护的情况下暴露在公网，举个例子，前段时间腾讯安全针对知名的 API 网关 Kong 作了一次安全分析，发现其主要问题就是 Admin API 缺乏安全防护，对于 APISIX 也是如此，所以再次建议大家，不要将 Admin API 直接暴露在公网，而是当做一条路由规则挂在 APISIXI 或者其他安全网关后，并补充相关安全防护措施。
